...
file_count = 0
with open(data_dir + "inter_test_projects.txt", "w") as f:
    for file in os.listdir(in_dir):
        if "DefinitelyTyped" in file:
            continue
        if os.stat(in_dir + "/" + file).st_size == 0:
...
# include words until count < threshold
...
with open(data_dir + "inter_source_wl", "w", encoding="utf-8") as out:
    for name, count in source_words:
        out.write(name)
        out.write("\n")

with open(data_dir + "inter_target_wl", "w", encoding="utf-8") as out:
...
train_file = data_dir + "inter_train.txt"
valid_file = data_dir + "inter_valid.txt"
test_file = data_dir + "inter_test.txt"
